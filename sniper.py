...
def composite_score(ev, slot="") -> float:
    """
    Pesi v2:
      s_fonte  25% — score fonte originale (eterogeneo ma informativo)
      s_buy    35% — tipo segnale: LONG_CONF/BUY1=100, LONG_EARLY/BUY2=66, BUY3=33
      s_nfonti 10% — numero fonti (basso: 1 fonte forte > 3 fonti deboli)
      s_mom1w  20% — momentum 1W con curva che picca a +10%, non premia chi ha già corso
      s_mom2w  10% — momentum 2W (sostituisce 4W, più sensibile ai giri recenti)
    Nessun bonus slot — non giustificato empiricamente.
    """
    ...
    if r1w is not None:
        base = min(100, max(0, r1w * 4 + 40))   # lineare fino a +15%
        if r1w > 15: base = min(85, base)         # cap soft sopra +15%
        s_mom1w = base
    else:
        s_mom1w = 40  # neutro penalizzante (non 50) — segnala assenza dati

    # Momentum 2W: stesso schema ma più morbido
    if r2w is not None:
        base2 = min(100, max(0, r2w * 3 + 40))
        if r2w > 20: base2 = min(85, base2)
        s_mom2w = base2
    else:
        s_mom2w = 40

    cs = s_fonte*0.25 + s_buy*0.35 + s_nfonti*0.10 + s_mom1w*0.20 + s_mom2w*0.10
    return round(cs, 1)
...
def run():
    ...
    # Proxy momentum da fonti (prima di yfinance)
    # priorità: mom1m scannerv2 (pulito) > perfSett leva/3 (proxy distorto)
    for ev in merged:
        if ev.get("ret_1w") is not None:
            continue  # già disponibile
        for src in ev.get("sources", []):
            ex_data = src.get("extra", {})
            src_name = src.get("source", "")
            # scannerv2: mom1m è già percentuale settimanale/mensile
            if src_name == "scannerv2" and ex_data.get("mom1m") is not None:
                ev["ret_1w"] = ex_data["mom1m"]
                ev["ret_2w"] = ex_data.get("mom3m")  # proxy 2W con mom3m
                ev["momentum_source"] = "scannerv2"
                break
            # leva: perfSett è performance del leva — dividi per ~2.5 come proxy
            if src_name == "leva" and ex_data.get("ret_1w") is not None:
                mult = 2.5  # approssimazione media leva
                ev["ret_1w"] = round(ex_data["ret_1w"] / mult, 2)
                ev["ret_2w"] = round(ex_data["ret_4w"] / mult, 2) if ex_data.get("ret_4w") else None
                ev["momentum_source"] = "leva_proxy"
                break
        if ev.get("ret_1w") is None:
            ev["momentum_source"] = "none"
