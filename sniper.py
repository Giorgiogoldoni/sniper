...
Confronta con storico, salva nuovi eventi in data/signals.json (ultimi 30 giorni).
"""
...
SOURCES = {
    ...
    "settoriali": {
        "url": "https://giorgiogoldoni.github.io/raptor-settoriali/data/portfolio.json",
...
def parse_chart(data: dict) -> list[dict]:
    """
    Format: data[] con campi ticker, name, signal, score, close, signal_date
    """
    results = []
...
def parse_alert(data: dict) -> list[dict]:
    """
    Format: data[] con campi ticker, nome, segnale, score, entryDate
    """
    results = []
    items = data if isinstance(data, list) else data.get("data", [])
    for item in items:
