...
def parse_scannerv2(data: dict) -> list[dict]:
    """
    Format: {meta:{}, signals:{ticker: {signal, score, ...}}} oppure {tickers:[]}
    """
    results = []
    # Prova struttura a dizionario {ticker: {signal, ...}}
    signals_dict = data.get("signals", {})
    if isinstance(signals_dict, dict) and signals_dict:
        for ticker, item in signals_dict.items():
            if not isinstance(item, dict):
                continue
            raw = item.get("signal", item.get("segnale", ""))
            sig = normalize_signal(raw)
            if not sig:
                continue
            results.append({
        ...
        return results
    # Prova struttura a lista
    items = data.get("tickers", data.get("data", data.get("results", [])))
