...
def parse_minfinder(data: dict) -> list[dict]:
    """
    Format: {top20:[{ticker, name, buy_level, composite_score, price,
                     inversion_score, trigger_count, kama_cross,
                     atr_rising, obv_divergence, sar_bull, ...}]}
    """
    results = []
    for item in data.get("top20", []):
        ...
        results.append({
            ...
            "price":       item.get("price"),
            "extra": {
                "inversion_score":  item.get("inversion_score"),
                "trigger_count":    item.get("trigger_count"),
                "kama_cross":       item.get("kama_cross"),
                "kama_cross_bars":  item.get("kama_cross_bars"),
                "atr_rising":       item.get("atr_rising"),
                "obv_divergence":   item.get("obv_divergence"),
                "sar_bull":         item.get("sar_bull"),
                "price_action_3up": item.get("price_action_3up"),
                "ret_1w":           item.get("ret_1w"),
                "ret_3m":           item.get("ret_3m"),
                "dist_52w_low":     item.get("dist_52w_low"),
                "is_min_storico":   item.get("is_min_storico"),
                "is_pullback":      item.get("is_pullback"),
                "is_compressione":  item.get("is_compressione"),
                "is_min_relativo":  item.get("is_min_relativo"),
                "borsa":            item.get("borsa"),
...
def merge_cross_repo(raw_items: list) -> list:
    ...
    for source, item, page_url in raw_items:
        ...
        # Salva price se disponibile
        if item.get("price") is not None and groups[key].get("price") is None:
